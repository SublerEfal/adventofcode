...
/*
    display: {
        digitCodes: ['abdefg', 'be', 'abcdf', 'bcdef', 'bceg', 'cdefg', 'acdefg', 'bde', 'abcdefg', 'bcdefg'], // Codes matching index
        codes: ['be', 'cfbegad', 'cbdgef', 'fgaecd', 'cgeb', 'fdcge', 'agebfd', 'fecdb', 'fabcd', 'edb'], // Scrambled codes input
        displayCodes: ['abcdefg', 'bcdef', 'bcdefg', 'bceg'], // Codes in display
        result: "8394" // String representation of end code
    }
*/

let displays = getDisplays();
displays.forEach(display => display.digitCodes = findDigitCodes(display.codes));
solveDisplays(displays);
console.log(displays.reduce((acc, display) => acc += parseInt(display.result), 0));


function solveDisplays(displays){
    displays.forEach(display => display.result = display.displayCodes.map(displayCode => display.digitCodes.indexOf(displayCode)).join(""));
}


function countDigits(displays, toCountDigits){
    return displays.reduce(function(acc, display){
        return acc + display.displayCodes.filter(code => toCountCodes.indexOf(code) > -1).length;
    }, 0);
}

function mapToSortedCode(code){
    return code.split("").sort((a,b) => a < b ? -1 : 1).join("");
}

function findDigitCodes(codes){
    //['be', 'cfbegad', 'cbdgef', 'fgaecd', 'cgeb', 'fdcge', 'agebfd', 'fecdb', 'fabcd', 'edb']
    let sortedCodes = codes.sort((a,b) => a.length - b.length).map(mapToSortedCode);
    //['be', 'bde', 'bceg', 'cdefg', 'bcdef', 'abcdf', 'bcdefg', 'acdefg', 'abdefg', 'abcdefg']
    let n1 = sortedCodes[0];
    let n4 = sortedCodes[2];
    let n7 = sortedCodes[1];
    let n8 = sortedCodes[9];
    let n3 = find3(n1, [sortedCodes[3], sortedCodes[4], sortedCodes[5]]);
    let n5 = find5(n3, n4, [sortedCodes[3], sortedCodes[4], sortedCodes[5]]);
    let n2 = find2(n3, n5, [sortedCodes[3], sortedCodes[4], sortedCodes[5]]);
    let n9 = find9(n5, n7, [sortedCodes[6], sortedCodes[7], sortedCodes[8]]);
    let n6 = find6(n5, n9, [sortedCodes[6], sortedCodes[7], sortedCodes[8]]);
    let n0 = find0(n6, n9, [sortedCodes[6], sortedCodes[7], sortedCodes[8]]);
    
    return [n0, n1, n2, n3, n4, n5, n6, n7, n8, n9];
    //['abdefg', 'be', 'abcdf', 'bcdef', 'bceg', 'cdefg', 'acdefg', 'bde', 'abcdefg', 'bcdefg']
}

function find3(n1, fives){
    return fives.filter(x => countMatchingDigits(n1, x) === 2)[0];
}

function find5(n3, n4, fives){
    return fives
        .filter(x => x !== n3)
        .filter(x => countMatchingDigits(n4, x) === 3)[0];
}

function find2(n3, n5, fives){
    return fives
        .filter(x => x !== n3)
        .filter(x => x !== n5)[0];
}

function find9(n5, n7, sixes){
    return sixes
        .filter(x => countMatchingDigits(n5, x) === 5)
        .filter(x => countMatchingDigits(n7, x) === 3)[0];
}

function find6(n5, n9, sixes){
    return sixes
        .filter(x => x !== n9)
        .filter(x => countMatchingDigits(n5, x) === 5)[0];
}

function find0(n6, n9, sixes){
    return sixes
        .filter(x => x !== n6)
        .filter(x => x !== n9)[0];
}
...
function countMatchingDigits(a, b){
    return a.split("").filter(x => b.indexOf(x) > -1).length;
}

function getDisplays(){
    let input = getInput().split("\n").map(x => x.trim());
    return input.map(x => x.split(" | ")).map(x => {return {codes: x[0].split(" "), displayCodes: x[1].split(" ").map(mapToSortedCode)}});
}

function getInput(){
    if(TEST) return `be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
    ...
    return `dacefg fegab de dceb bedag dae bcgaefd bdacg fbgcad bgedca | acfebgd de dbagc deagcb
