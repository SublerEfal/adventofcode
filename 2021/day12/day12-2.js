...
function findAllPaths(cave, paths=[], path=[]){
    ...
    let isCurrentDoubleSmallCave = false;
    cave.connections.forEach(nextCave => {
        if(isSmallCave(nextCave) && path.indexOf(nextCave.name) > 0){
            if(path.currentDoubleSmallCave) return;
            isCurrentDoubleSmallCave = true;
            path.currentDoubleSmallCave = true;
        }
        findAllPaths(nextCave, paths, path);
        if(isCurrentDoubleSmallCave){
            path.currentDoubleSmallCave = false;
        }
    });
    path.pop();
    return paths;
